...
def test_smoke_table_reference_other_database():
    ...
    def in_table():
        table_store = ConfigContext.get().store.table_store
        database = "pipedag_external"
        schema = Schema("user_controlled_schema", prefix="", suffix="")
        db_schema = Schema(f"{database}.{schema.get()}", prefix="", suffix="")
        table_name = "external_table"
        try:
            with table_store.engine_connect() as conn:
        ...
        table_store.execute(
            CreateTableAsSelect(
                ...
                db_schema,
                query,
                quote_schema=False,  # causes trouble with database part
            )
        )
        return Table(ExternalTableReference(table_name, schema=db_schema.get()))
    ...
    with Flow() as f:
        ...
        with Stage("reference_constant"):
            ...
            # recommended to reference any table generated by pipedag code this way.
            # This just helps produce test input for this feature test.
            table3 = Table(ExternalTableReference("external_table", schema="pipedag_external.user_controlled_schema"))
            table4 = duplicate_table_reference(table3)
            _ = m.assert_table_equal(table3, table4, check_dtype=False)
            table5 = m.noop(table3)
            table6 = m.noop_lazy(table4)
            _ = m.assert_table_equal(table5, table4, check_dtype=False)
            _ = m.assert_table_equal(table6, table4, check_dtype=False)
