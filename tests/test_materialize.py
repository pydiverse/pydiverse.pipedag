...
def create_view_view1(tbl: Alias):
    return Table(View(src=tbl, columns=SortCol(tbl.c.c1, SortOrder.DESC)))


@materialize(input_type=sa.Table, lazy=True)
def create_view_view2(tbl: Alias):
    return Table(View(src=tbl, sort_by=tbl.c.col1, limit=3))
...
def test_materialize_view(imperative):
    ...
    with Flow("flow") as f:
        with Stage("stage"):
            ...
            ll_lazy = _m.noop_lazy(l)
            nn_lazy = _m.noop_lazy(n)
            _ = yy, zz, ll, nn, yy_lazy, zz_lazy, ll_lazy, nn_lazy
