...
class BaseViewMaterializationHook(TableHook[SQLTableStore], abc.ABC):
    ...
    def fuse_sub_views(cls, *, view: View, sub_view: View):
        ...
        def check_missing(cols, sub_cols):
            if any(col not in sub_cols for col in cols):
                ...
                raise ValueError(
                    f"Cannot fuse views because columns {missing} are not present in the sub view. Table: {table}"
                )
        ...
        def translate(cols, sub_cols):
            ...
            if isinstance(cols, Mapping):
                ...
                return {name: sub_cols.get(col, col) for name, col in cols.items()}
            if isinstance(cols, Iterable) and not isinstance(cols, str):
                check_missing(cols, sub_cols)
                return [sub_cols.get(col, col) for col in cols]
            check_missing([cols], sub_cols)
            return sub_cols.get(cols, cols)
    ...
    def replace_view_src_tables(cls, src: Iterable[Any] | Any, input_table_mapping: dict[Any, Table]):
        ...
        msg = (
            ...
            "exact table refernces as you received them as task inputs. "
        )
        if isinstance(src, Iterable):
            ...
            replaced_src = [map[0] for map in replaced_src]  # drop second tuple result
    ...
    def materialize(
        ...
        columns, sort_by = cls.update_column_references(in_out_table=table)
        if hasattr(table.view.src, "view") and table.view.src.view:
            # fuse views if the source is itself a view
            cls.fuse_sub_views(in_out_table=table)
        return
